...
public:
	...
	__forceinline Matrix3(Vector3 x, Vector3 y, Vector3 z) 
	{ 
		m_mat[0] = x; 
		m_mat[1] = y; 
		m_mat[2] = z; 
	...
	__forceinline Matrix3(Quaternion q) 
	{ 
		*this = Matrix3(XMMatrixRotationQuaternion(q)); 
	}

	__forceinline explicit Matrix3(const XMMATRIX& m) 
	{ 
	...
	__forceinline explicit Matrix3(EIdentityTag) 
	{ 
	...
	__forceinline explicit Matrix3(EZeroTag) 
	{ 
	...
	__forceinline operator XMMATRIX() const { return (const XMMATRIX&)m_mat; }
